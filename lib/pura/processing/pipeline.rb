...
module Pura
  module Processing
    class Pipeline
      ...
      def create_tempfile
        tempfile = Tempfile.new(["pura_processing", ".#{destination_format}"], binmode: true)
      ...
      # In case of processing errors, processors may leave an empty destination
      # file they created, so this method makes sure it is deleted in case an
      # exception is raised on saving the image.
      ...
      def determine_format(file_path)
        ...
        extension[1..] if extension.size > 1
      end
    end
  end
end
